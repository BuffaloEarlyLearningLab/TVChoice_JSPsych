document.addEventListener('DOMContentLoaded', function() {
    var jsPsychInstance = initJsPsych({
		on_trial_start: function() {
			...
			var order = jatos.urlQueryParameters.order;
			console.log(order);
			jsPsychInstance.data.addProperties({order: order});
			jatos.studySessionData.order = order;
            console.log("Order retrieved from session data: ", jatos.studySessionData["order"]);
		},
    ...
    indices.forEach(function(index) {
        ...
        var speaker_audio_trial = {
			...
			on_finish: function() {
				...
				var resultData = {"id": jatos.studySessionData.id, "order": jatos.studySessionData.order, "selected": speaker.audio, "trial": index}
